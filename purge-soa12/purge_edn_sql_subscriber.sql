create or replace
PROCEDURE PURGE_EDN_SQL_SUBSCRIBER (start_time IN timestamp, end_time IN timestamp)  AS
  ...
  total              int;
BEGIN
  sub := 'EDN_SQL_SUBSCRIBER';
  log_id := edn_next_log_event_id;
  total := 0;
  dbms_output.put_line('--- Start ---');  
  for rec in
       ( SELECT MSGID 
       FROM AQ$_EDN_EVENT_QUEUE_TABLE_I
       WHERE TRUNC(msg_enq_time) BETWEEN TRUNC(start_time) AND TRUNC(end_time) 
           AND  SUBSCRIBER# in (SELECT subscriber_id FROM AQ$_EDN_EVENT_QUEUE_TABLE_S WHERE name = sub)
       )
  ...
  dbms_output.put_line('--- Done purge. Total: ' || total || '---') ;
